...
const RestaurantCard = (props) => {
   ...
   // console.log
    const { resData } = props;
    const { areaName, name, avgRating, cloudinaryImageId, sla, cuisines, availability } = resData?.info;
    const { slaString } = sla;
  ...
  const truncateResName = (str) => {
    ...
    return (
      ...
      <div className='w-80 h-56 card relative rounded-xl'>
          <img src={CDN_URL + cloudinaryImageId} alt="res-img" className='rounded-xl w-full h-full object-cover' />
      </div>
      <div className='ml-3'>
...
export const withOfferLabel = (RestaurantCard)=>{
  ...
  //  console.log(info,"infoby")
   const { aggregatedDiscountInfoV3, aggregatedDiscountInfoV2 } = props?.resData?.info;
        ...
        return (
            <>
                ...
                {
                    aggregatedDiscountInfoV2 && <div className='absolute bottom-28 left-3 mb-5 text-white uppercase font-ProximaNovaBlack tracking-tighter text-[22px] label'>{aggregatedDiscountInfoV2?.header}</div>
                }
                {
                    aggregatedDiscountInfoV3 && <div className='absolute bottom-28 left-3 text-white uppercase font-ProximaNovaBlack tracking-tighter text-[22px] label'>{aggregatedDiscountInfoV3?.header} {aggregatedDiscountInfoV3?.subHeader}</div>
                }
            </>
        )
  }
